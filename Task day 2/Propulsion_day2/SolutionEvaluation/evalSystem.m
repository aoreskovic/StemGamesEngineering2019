function [qmG_kg_s, Turbine_power_kW, theta_fg1_degC, theta_fg2Evap_degC,...
    ...
    system_efficiency, CombTempFlag, TurbTempFlag, SaturatedFlag, deadFlag, evapErosionFlag, evapSuperFlag, pumpSpeedExceedFlag, maxDepth] = ...
    evalSystem(turbID, pipeLen, evNr, supNr, compID, pumID, N1, L1, N2, L2, fgFrac, pSpeed, depth, mFuel)
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here

%fuel tank parameters
Sub.Fuel.Tank.initMass = mFuel; %kg, initial fuel mass
...
%% INPUTS
pump_speed_vector = [ones(1,100)*pSpeed];
depth = ones(1,length(pump_speed_vector))*depth;
...
if sub.Steam.Turb.xOut < 0.9
    SaturatedFlag = true;
else
    SaturatedFlag = false;
end

if sub.FG.Comb.Tout > 850
    CombTempFlag = true;
else
    CombTempFlag = false;
end

if sub.Steam.HE.Super.Tout > sub.Turb.Tmax
    TurbTempFlag = true;
else
    TurbTempFlag = false;
end

if sub.Turb.Power < sub.Env.Pmin
    deadFlag = true;
else
    deadFlag = false;
end
...
if sub.HE.Evap.ero 
    evapErosionFlag = true;
else
    evapErosionFlag = false;
end
...
if pSpeed > sub.Fuel.Pump.Nallowed
    pumpSpeedExceedFlag = true;
else
    pumpSpeedExceedFlag = false;
end
