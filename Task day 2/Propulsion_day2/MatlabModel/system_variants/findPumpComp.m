function [pumpsFeasible,comprFeasible] = findPumpComp(turbineNR, pipeNR, velocity_evap, velocity_cond)
...
for i = 1:length(Fpumps)
    if Qpump >= Fpumps(i,4)* Fpumps(i,2) && Qpump <= Fpumps(i,2)
        pumpsFeasible = [pumpsFeasible, i];
    end
end
...
for j = 1:length(Comps)
    if qmFGret_kg_h >= Comps(j,2) &&  qmFGret_kg_h<= Comps(j,3)
        comprFeasible = [comprFeasible, j];
    end
end
