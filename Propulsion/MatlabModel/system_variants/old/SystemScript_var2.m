...
turbineNR = 2; %selected turbine
pipeNR = 1; %selected pipe
pipeLength = 2.25; %evaporator and condenser, m
numberOfPipesEvap = 123; %number of tubes in evaporator
AlphaEvapSteam =3500; 
evapRatio = 0.94049;
numberOfPipesSuper = 41;
AlphaSuperSteam = 70;
...
depth = ones(1,length(pump_speed_vector))*30;
...
for i=1:times
    ...
    qmRet = compressor(pump_speed, qmRetMin,qmRetMax, qmRetlow, qmRethigh, Nretlow, Nrethigh);
    [theta_fg1, qn_H2O, qn_CO2] = Combustion_chamber( qmG, theta_ret, qmRet);
...
Results = table(Time_s, qmG_kg_s, fuel_kg, Q_l_h, h_m, Turbine_power_kW, theta_fg1_degC,...
