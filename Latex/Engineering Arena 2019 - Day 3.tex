...
Your main task for today is to control the submarine. In order for your submarine to successfully sail, it is necessary to have properly calculated the stability of the submarine. Stability of the submarine must be independent of the cargo or the people in the submarine. In other words, the submarine must be able to sail independently of its mass. The main task of your submarine is to supply and maintain underwater habitats which means that the communication system between submarine and each habitat must be set. The submarine is able to navigate to the corresponding habitats using the continuous communication between the submarine and each habitat. The last task is to define the governing system which will control the submarine position.
...
Drawing of a submarine is shown in figure \ref{}. %dodaj sliku.
